  ...
  // Draws everything
  void drawAll(float[] sum,boolean playing){//,color f, color b) {
    colorMode(RGB);
    ...
    while (x < width * 1.5 && x > 0 - width / 2) {
      ...
      fill(0);
     circle(center.x,center.y-75,sphereRadius*2.001);
      for (float angle = 0; angle <= 360; angle += 1.5) {
        ...
        noStroke();
        //strokeWeight(2);
        float h = map(surroundingRadius, surrRadMin, surrRadMax, 0,100);
        fill(map(surroundingRadius, surrRadMin, surrRadMax, 0,100));
        fill(180,h,h);
        //fill(angle+90,100,100);
        circle(x, y, 5);
         if(!playing)
        break;
        //noFill();
    ...
    for (int angle = 0; angle <= 240; angle++) {
      ...
      /*if (sum[0] != 0) {
        if (angle >= 0 && angle <= 30) {
          extendingSphereLinesRadius = map(sum[360 - round(map((angle), 0, 30, 0, 80))], 0, 0.8, extendingSphereLinesRadius - extendingSphereLinesRadius / 8, extendingLinesMax * 1.5); // Highs
        } else if (angle > 30 && angle <= 90) {
          extendingSphereLinesRadius = map(sum[280 - round(map((angle - 30), 0, 60, 0, 80))], 0, 3, extendingSphereLinesRadius - extendingSphereLinesRadius / 8, extendingLinesMax * 1.5); // Mids
        } else if (angle > 90 && angle <= 120) {
          extendingSphereLinesRadius = map(sum[200 - round(map((angle - 90), 0, 30, 65, 80))], 0, 40, extendingSphereLinesRadius - extendingSphereLinesRadius / 8, extendingLinesMax * 1.5); // Bass
        } else if (angle > 120 && angle <= 150) {
          extendingSphereLinesRadius = map(sum[120 + round(map((angle - 120), 0, 30, 0, 15))], 0, 40, extendingSphereLinesRadius - extendingSphereLinesRadius / 8, extendingLinesMax * 1.5); // Bass
          ...
          //extendingSphereLinesRadius = map(sum[0 + round(map((angle - 210), 0, 30, 0, 15))], 0, 40, extendingSphereLinesRadius - extendingSphereLinesRadius / 8, extendingLinesMax * 1.5); // Bass
         else if (angle > 270 && angle <= 330) {
          extendingSphereLinesRadius = map(sum[80 + round(map((angle - 270), 0, 60, 0, 80))], 0, 3, extendingSphereLinesRadius - extendingSphereLinesRadius / 8, extendingLinesMax * 1.5); // Mids
        } else if (angle > 330) {
          extendingSphereLinesRadius = map(sum[160 + round(map((angle - 330), 0, 30, 0, 80))], 0, 0.8, extendingSphereLinesRadius - extendingSphereLinesRadius / 8, extendingLinesMax * 1.5); // Highs
        }*/
        if (sum[0] != 0) {
      if (angle >= 0 && angle <= 30) {
      ...
      float h = (map(extendingSphereLinesRadius, extendingLinesMin-100, extendingLinesMax+20, 0, 360));
      strokeWeight(2);
      stroke(h,100,100);
      if (y <= getGroundY(x)) {
        line(x, y, xDestination, yDestination);
